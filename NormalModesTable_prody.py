...
class NormalModeTraj:
    ...
    def __len__(self):
        return len(self.molecule.numCoordsets)
...
#   Move is incompletely implemented
    ...
    def __init__(self, mol=None, modes=None, sesData=None, *args, **kw):
        ...
        if modes:
            ...
            print modes
            self.molecule = mol
            self.title = "Normal Modes for %s" % mol.getTitle()
            for atom in iter(mol):
...
#       self.scaleWidget.pack(expand=True, fill="both")
    ...
    def _createMovieDialog(self, m):
        ...
        try:
            cs2 = m.getCoordsets(1)         # + displacement
        except IndexError:
            cs2 = m.addCoordset(m.getCoords())

        try:
            cs3 = m.getCoordsets(2)         # 0 displacement
        except IndexError:
            cs3 = m.addCoordset(m.getCoords())

        try:
            cs4 = m.getCoordsets(3)         # - displacement
        except IndexError:
            cs4 = m.addCoordset(m.getCoords())

        # newcoords = numpy.array([m.getCoords()[i] + modes[0].getEigvec()[i*3:i*3+3]
        #    for i in range(0,len(m.getCoords()))])
        # m.addCoordset(newcoords)
        # m.addCoordset(m.getCoords())
        # newcoords = numpy.array([m.getCoords()[i] - modes[0].getEigvec()[i*3:i*3+3]
        #    for i in range(0,len(m.getCoords()))])
        # m.addCoordset(newcoords)
        ...
        ensemble.endFrame = m.numCoordsets()
        ensemble.molecule = m
    ...
    def _updateTrajectory(self, nmp, triggerName, ignore):
        ...
        if nmp is not None:
            m.setACSIndex(1)
            newcoords = numpy.array([m.getCoordsets(0)[i] + sf*nmp.displacements[i]
                                     for i in range(0, len(m.getCoordsets(0)))])
            m.setCoords(newcoords)

            m.setACSIndex(2)
            m.setCoords(m.getCoordsets(0))
    ...
    def _drawVector(self, nmp):
        if self.__recursion:
            ...
            self.__recursion = True
            slef.movieDialog.ensemlbe.molecule.setACSIndex(0)
        try:
            chimera.openModels.close(self.__bildModel)
        except:
            pass
        ...
        m = self.movieDialog.ensemble.molecule
        # cs1 = m.getCoordsets(1)  # 0? Ja l'hem posat a zero abans
        displacement_vectors = nmp.displacements
        dict_vectors = dict()
        for dv, atm in zip(displacement_vectors, m.iterAtoms()):
            # for dv, atm in zip(displacement_vectors, m.atoms): # si iterem sobre m
            # ens retorna els atoms
            dict_vectors[atm] = dv
        ...
        if self.representation == 1:
            # for atm in m.atoms:
            #      x = dict_vectors[atm][0] * sf
            #      y = dict_vectors[atm][1] * sf
            #      z = dict_vectors[atm][2] * sf
            #      v = Vector(x, y, z)
            for v, atm in zip(displacement_vectors, m.iterAtoms()):
                # for v, atm in zip(displacement_vectors, m.atoms):
                v *= sf
        ...
        if self.representation == 2:
            for atm in iter(m):
                if str(atm.getElement()) == "H":
                    v = Vector(0, 0, 0)
                else:
                    v = dict_vectors[atm] * sf
                    # x = dict_vectors[ma][0] * sf
                    # y = dict_vectors[ma][1] * sf
                    # z = dict_vectors[ma][2] * sf
                    # v = Vector(x, y, z)
                    if str(v) == "0 0 0":
                        pass
                    else:
        ...
        if self.representation == 3:
            ...
            #     else:
            #         print >> bild, ".arrow "+str(p[0])+" "+str(p[1])+" "+str(p[2])+" "+str(p[0]-v[0])+" "+str(p[1]-v[1])+" "+str(
            # p[2]-v[2])+" %.2f %.2f %.2f" % (float(self.__vectorCyl.get()),
            # float(self.__vectorCon.get()), float(self.__vectorRel.get()))
